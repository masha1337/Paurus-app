...
export class TableComponent {
    ...
    updateRow(rowData: Student) {
        this.tableService.updateItem(rowData).then((res) => {
            this.alertService.displayAlertChange({type: 'success', message: `Student "${rowData.firstName} ${rowData.lastName}" successfully updated.`});
        })
        .catch((error) => {
    ...
    deleteRow(rowData: Student) {
        this.tableService.deleteItem(rowData).then((res) => {
            this.alertService.displayAlertChange({type: 'success', message: `Student "${rowData.firstName} ${rowData.lastName}" successfully deleted.`});
        })
        .catch((error) => {
            this.alertService.displayAlertChange({type: 'error', message: error.message});
        });
